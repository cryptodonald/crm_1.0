...
function KanbanRoot<T>(props: KanbanRootProps<T>) {
  ...
  // Annunci localizzati in italiano
  const announcements: Announcements = React.useMemo(
    () => ({
      onDragStart({ active }) {
        const isColumn = active.id in value;
        const tipo = isColumn ? "colonna" : "elemento";
        const posizione = isColumn
        ...
        return `Selezionato ${tipo} alla posizione ${posizione} di ${totale}`;
      },
      onDragOver({ active, over }) {
        ...
        const colOver = getColumn(over.id);
        const colActive = getColumn(active.id);

        if (isColumn) {
          return `${tipo} ora alla posizione ${posizione} di ${totale}`;
        }

        if (colActive !== colOver) {
          return `${tipo} ora alla posizione ${posizione} di ${totale} in ${colOver}`;
        }
      ...
      onDragEnd({ active, over }) {
        ...
        if (isColumn) {
          return `${tipo} rilasciata alla posizione ${posizione} di ${totale}`;
        }

        if (colActive !== colOver) {
          return `${tipo} rilasciato alla posizione ${posizione} di ${totale} in ${colOver}`;
        }

        return `${tipo} rilasciato alla posizione ${posizione} di ${totale}`;
      },
      onDragCancel({ active }) {
        const isColumn = active.id in value;
        const tipo = isColumn ? "colonna" : "elemento";
        return `Trascinamento annullato. ${tipo} rilasciato.`;
      },
    }),
      ...
      <DndContext
        ...
        accessibility={{
          ...
          screenReaderInstructions: {
            draggable: `
            Per selezionare un elemento o una colonna, premi spazio o invio.
            Durante il trascinamento, usa i tasti freccia per spostare l'elemento.
            Premi di nuovo spazio o invio per rilasciare l'elemento nella nuova posizione, oppure premi ESC per annullare.
          `,
          },
          ...accessibility,
        }}
