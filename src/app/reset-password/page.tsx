...
function ResetPasswordContent() {
  ...
  useEffect(() => {
    if (!token) {
      setErrors({
        general: 'Token di reset password non valido o mancante.'
      });
    }
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        setIsSuccess(true);
        setSuccessMessage(result.message || 'Password aggiornata con successo!');
        ...
        setErrors({
          general: result.error || 'Errore durante l\'aggiornamento della password'
        });
      }
    } catch (error) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-green-600">Password Aggiornata!</h1>
          </div>
