...
import { getLeadAttributions } from '@/lib/seo-ads/queries';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters = {
      source: (searchParams.get('source') as AttributionSource) || undefined,
      confidence: (searchParams.get('confidence') as AttributionConfidence) || undefined,
    ...
    const result = await getLeadAttributions(filters);
    return NextResponse.json({
      attributions: result.data,
      total: result.pagination.total,
      pagination: result.pagination,
    });
  } catch (error: unknown) {
    console.error('[API] GET /api/seo-ads/attribution error:', error);
    return NextResponse.json(
