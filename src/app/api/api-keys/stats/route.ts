  ...
  try {
    ...
    if (keyId) {
      ...
      return NextResponse.json({
        ...
        dailyAverage: Math.round(usageStats.reduce((sum, stat) => sum + stat.count, 0) / days),
      });
    } else {
      ...
      const previousWeekUsage = previous7Days.reduce((sum, usage) => sum + usage, 0);
      const usageTrend = previousWeekUsage > 0 
