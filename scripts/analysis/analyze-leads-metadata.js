...
async function getAirtableKey() {
  try {
    ...
    const response = await fetch(
      'http://localhost:3000/api/api-keys?stats=false'
    );
    const data = await response.json();

    // Trova la chiave Airtable
    const airtableKey = data.apiKeys?.find(
      key => key.service === 'airtable' && key.isActive
    );
...
async function getTableSchema(apiKey) {
  ...
  try {
    ...
    if (!schemaResponse.ok) {
      throw new Error(
        `Schema API error: ${schemaResponse.status} - ${await schemaResponse.text()}`
      );
    }
    ...
    if (!leadsTable) {
      console.log(
        '📋 Tabelle disponibili:',
        schemaData.tables?.map(t => ({ id: t.id, name: t.name }))
      );
      throw new Error('Tabella leads non trovata');
    }

    console.log(
      `✅ Tabella trovata: "${leadsTable.name}" (ID: ${leadsTable.id})`
    );
    return leadsTable;
  } catch (error) {
  ...
  fields.forEach((field, index) => {
    ...
    if (field.options) {
      if (field.options.choices) {
        console.log(
          `   Opzioni: ${field.options.choices.map(c => c.name).join(', ')}`
        );
      }
...
function analyzeForKPIs(fields, sampleData) {
  ...
  const contactedField = fields.find(
    f =>
  ...
  console.log(
    `   Campo utilizzabile: ${createdField ? createdField.name : '❌ Campo data creazione non trovato'}`
  );

  console.log('\n🎯 KPI: Contattati entro 48h');
  console.log(
    `   Campo utilizzabile: ${contactedField ? contactedField.name : '❌ Campo contatto non trovato'}`
  );

  console.log('\n🎯 KPI: Tasso di qualificazione');
  console.log(
    `   Campo utilizzabile: ${statusField ? statusField.name : '❌ Campo stato non trovato'}`
  );
...
async function main() {
  ...
  const REAL_API_KEY =
    'patKEe4q8UeW13rVL.5f05611c9f7dbfc41c4196bfb1ae8cb52bde23ad4c6b9e30678027ac94c1a6ea';
